...
public class BinarySearchTreeQ1 {
    ...
    public boolean isValidBST(){
        List<Integer> orderList = DFSInOrder();
        for(int i=0 ; i<orderList.size();i++){

            if(i+1 == orderList.size()) return true;
            Integer nexNum = orderList.get(i+1);
            Integer currNum = orderList.get(i);

            if(currNum>nexNum) return false;
        }
        return true;
    }
